...
def add_beer(dico, beer_name, stock, buy_price, coef_down, coef_up, coef_max):
    dico[beer_name] = {'stock': stock, 'buy_price': buy_price, 'price': buy_price, 'coef_down': coef_down,
                       'coef_up': coef_up, 'coef_max': coef_max, 'q_qarder': [], 'q_current_qarder': 0}
...
def update(dico, beer_name):
    if beer_name in dico.keys():  # check if the beer is listed
        ...
        if beer['q_qarder'] != []:                                      # check if the list of quantity by for each qarder is not empty
            ...
            beer['price'] = compute_new_price(beer['buy_price'], beer['price'], q_last_qarder,
                                              beer['q_current_qarder'], beer['coef_up'], beer['coef_down'], beer['coef_max'])
